...
def request_quotations(order_id):
    """Request quotations from all active vendors"""
    user_id = int(get_jwt_identity())
    ...
    # Simulate quotations from vendors (in production, would send actual requests)
    import random
    
    created_quotations = []
    for vendor in vendors:
        # Simulate vendor pricing (varies by +/- 20% from order total)
        base_price = order.total if order.total > 0 else 1000
        variation = random.uniform(0.8, 1.2)
        quoted_price = base_price * variation
        
        quotation = VendorQuotation(
            purchase_order_id=order_id,
            vendor_id=vendor.id,
            quoted_items=order.items,
            total_price=round(quoted_price, 2),
            delivery_days=vendor.delivery_days + random.randint(-1, 2),
    ...
    return jsonify({
        'message': f'Requested quotations from {len(vendors)} vendors',
